...
export default {
    ...
    data() {
        return {
            ...
            modifiedSchemeContainer: new SchemeContainer(this.scheme, null, this.editorId, 'edit', this.$store.state.apiClient)
        };
    },

    beforeMount() {
        if (this.originScheme) {
            this.originSchemeContainer = new SchemeContainer(this.originScheme, null, this.editorId, 'edit', this.$store.state.apiClient);
    ...
    methods: {
        ...
        loadBaseScheme(id) {
            ...
            .then(originScheme => {
                ...
                this.originSchemeContainer = new SchemeContainer(originScheme.scheme, null, this.editorId, 'edit', this.$store.state.apiClient);
                this.patch = generateSchemePatch(this.originSchemeContainer.scheme, this.scheme);
            })
            .catch(err => {
                console.error(err);
